# 박세종 (Park Sejong) · lifedesigner88

> 설계하고, 만들고, 가르치는 것 — 그 세 가지가 하나로 연결된 삶을 살고 있습니다.

---

## ✅ 저는 이런 사람입니다

대학교 때 조울증을 겪으며 방황하던 시절, 저는 한 가지 질문을 붙잡았습니다.

**"어떻게 사는 것이 사람답게 사는 것일까?"**

그 질문이 저를 청년 커뮤니티 기획자로, 12개국 국제캠프 PM으로,
수학·입시 교육자로, 그리고 2023년(만 35세에) 개발자로의 도전으로 이끌었습니다.

지금은 그 모든 경험을 하나로 합쳐, 교육 플랫폼 **세종클래스**를 혼자 만들어 운영하고 있습니다.
현재는 창업을 통한 좋은 일자리 만들기라는 도전을 이어가고 있습니다. 

---

## ✅ 코드 바깥의 저

뇌과학 책을 180권 넘게 읽었습니다. 
의식이 어떻게 생겨나는지, 감정이 몸에서 어떻게 만들어지는지가 궁금해서요. 
안토니오 다마지오의 책은 전작을 탐독했고, 조지프 르두와 유발 하라리도 마찬가지입니다. 

물리학도 독학으로 대학 교재 수준까지 팠고, 
박문호의 자연과학세상(박자세) 정회원으로 활동하고 있습니다. 

코드를 잘 짜고 싶은 이유도 결국 같습니다. 
좋은 시스템을 만들어 사람의 성장을 돕고 싶어서입니다. 

---

## ✅ 저에 대해 더 알고 싶다면

이 프로필은 요약본입니다. 19년치 삶을 구조화한 데이터 저장소를 따로 운영하고 있습니다.

| | |
|---|---|
| 📂 전체 데이터 | [hupository](https://github.com/lifedesigner88/lifedesigner88/tree/main/hupository) |
| 📅 19년 연도별 회고 | [timeline/](https://github.com/lifedesigner88/lifedesigner88/tree/main/hupository/data/timeline) |
| 📚 독서 목록 180권 | [reading/](https://github.com/lifedesigner88/lifedesigner88/tree/main/hupository/data/05_interests/reading) |
| 🗂️ 서사 챕터 6편 | [story/chapters/](https://github.com/lifedesigner88/lifedesigner88/tree/main/hupository/data/04_story/chapters) |

--- 

## ✅ 이런 분과 이야기하고 싶습니다

- 뇌과학·물리·수학·AI 중 하나라도 관심이 겹치는 분
- 교육 또는 학습 자체를 진지하게 생각해본 적 있는 분
- 기술로 무언가 의미 있는 걸 만들고 싶은 분
- 깊은 대화와 긴 호흡의 협업을 좋아하는 분

커피챗, 스터디, 사이드 프로젝트 언제든 환영합니다.

📬 lifedesigner88@gmail.com
📝 [블로그](https://blog.naver.com/lifedesigner88) · [세종클래스](https://sejongclass.kr) · [YouTube](https://www.youtube.com/@sejongclass)



...
- `data/09_journal/{이번주}/session.md`  ← 이번 주 세션이 있으면 포함
...
[대화 흐름]
1. "오늘 하루를 한 줄로 표현하면 어때?" 로 시작한다.
2. 오늘 가장 에너지를 많이 쓴 일이나 생각을 묻는다.
3. snapshot.yaml 의 active_tracks 중 오늘 관련 있었던 트랙만 자연스럽게 확인한다.
4. 오늘 내린 결정, 배운 점, 걸리는 점이 있었는지 묻는다.
5. 오늘 활동이 lifetime.yaml / tree.yaml 과 어떻게 이어지는지 한 문장으로 정리하도록 돕는다.
6. "내일 가장 중요한 한 가지는 뭐야?"를 물어 tomorrow_focus 를 뽑는다.

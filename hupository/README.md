...
작동 규칙 전체를 공개하는 저장소가 아니라,
박세종이 어떤 사람이고 지금 무엇에 집중하고 있는지 외부 독자가 빠르게 이해할 수 있도록 정리한 공개 데이터 레이어입니다.
...
- [data/05_interests/interests.yaml](data/05_interests/interests.yaml)

왜 이런 방향으로 움직이는지, 사고의 뿌리와 변화 맥락까지 보고 싶다면 이 경로가 좋습니다.

## 30초 소개

- 자신의 삶을 객관적으로 기록해 후대와 인공지능이 읽을 수 있는 생애 데이터로 남기려는 사람입니다.
- 교육 현장에서 오래 사람을 가르치고 돕다가, 지금은 교육과 기술을 연결하는 빌더로 전환 중입니다.
- 현재의 핵심 축은 `Hupository 생애 기록`, `SW마에스트로 17기 Team 310`, `휴포짓`, `세종클래스`, `AI builder`입니다.
- 결과물만 보여주는 포트폴리오보다, 방향과 판단의 맥락까지 함께 드러내는 방식을 더 중요하게 생각합니다.

조금 더 압축하면 이런 사람입니다.

> 자신의 삶을 데이터로 남기는 사람.
> 그 기록을 바탕으로 지식과 성장 구조를 다음 사람에게 상속하려는 교육자형 빌더.

## 지금 집중하는 것

2026-07-20 기준, 8월 20-21일 SW마에스트로 중간발표까지는 아래 순서로 운영합니다.

- `Hupository 생애 기록`
  박세종의 삶을 후대와 인공지능이 읽을 수 있는 장기 데이터로 기록합니다.
  최상위 목적은 삶의 객관 기록, 지식 상속, 그리고 인생을 아름답게 살아가는 방법에 대한 탐구입니다.
- `SW마에스트로 17기 팀 프로젝트`
  Team 310에서 `휴포짓`의 핵심 사용자 흐름, 동작하는 제품 데모, 고객 검증 근거와 발표 서사를 연결하고 있습니다.
  2026년은 2027-2037년 창업가 성장 인프라와 300명 일자리 생태계 목표를 준비하는 학습 기간입니다.
- `세종클래스`
  수학·물리·코딩·라이프디자인을 연결하는 1인 교육 플랫폼을 직접 기획·개발·운영하고 있으며, 당분간은 유지 모드로 관리합니다.
- `Hupository / AI builder`
  개인 맥락을 구조화해 자기이해와 다음 행동을 돕는 AI 시스템으로 발전시키고 있습니다.
  공개 mirror에서는 그중 외부에 공개 가능한 현재 상태와 판단만 읽을 수 있습니다.
- `기록 운영`
  중간발표까지는 상세 daily 대신 확정 일정, 중요한 결정, 다음 주에도 유효한 상태 변화만 최소 기록합니다.
...
raw journal 과 코칭 세션 원본은 상위 private repository 의 `private/journal/raw/`에서 관리합니다.
이 public mirror에서는 공개 가능한 구조와 현재 상태를 읽는 데 필요한 정보만 공개합니다.
...
- `hupository/` 안의 모든 파일은 실제 공개 파일이며, 비공개 원문은 상위 private companion으로 물리적으로 분리합니다.
